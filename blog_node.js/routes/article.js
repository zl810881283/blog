...
var storage = multer.diskStorage({
    destination: function (req, file, cb) {
        cb(null, path.join(globalSetting.dataPath, 'picture'));
    },
...
router.post('/add', upload.single('article[picture]'), function (req, res, next) {
    ...
    var article = new Article({
        ...
        picturePath: path.replace(globalSetting.dataPath, "/").replace(/\\/g, '/').replace('//', '/'),
...
router.post('/modify/:aid', upload.single('article[picture]'), function (req, res, next) {
    ...
    Article.findById(aid, function (err, article) {
        ...
        if (req.file) {
            ...
            article.picturePath = path.replace(globalSetting.dataPath, "/").replace(/\\/g, '/').replace('//', '/');
        }
...
router.get('/list/:pageNo', require('../utils/hotTop'), require('../utils/newestComments'), function (req, res, next) {
    //req.local.title='首页';
    var pageNo = req.params.pageNo || 1;
    var pageSize = globalSetting.pageSize;

    Article.count(function (err, count) {
        if (err) {
            next(err);
        }
        req.local.allPageNum = parseInt(count / pageSize) + 1;
        req.local.pageNavRange = utils.calaPageNavRange(pageNo, req.local.allPageNum, globalSetting.pageNavSize);

        req.local.currentPageNum = pageNo;

        Article.fetchAndPopulateByPage(pageNo, pageSize, function (err, articles) {
            if (err)
                return next(err);
            articles.forEach(function (article) {
                ...
                article._content = utils.genSummary(article.content);
